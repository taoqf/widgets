...
**MMStudio widgets** now supports a variety of themes, and it is very easy to create or inherit modifications, no third-party styles or configs.
...
<Note>
	Before using any Components, you must add <Code>MMProvider</Code> to the root node.
</Note>
<Spacer y={0.5} />
<Note type="success">
...
1. Make sure `MMProvider` and `CssBaseline` are already on the root component.

2. Update the value of `themeType`, and the theme of all components will follow automatically.

```jsx
import { CssBaseline, MMProvider } from '@mmstudio/widgets';
...
const App = () => {
	...
	return (
		<MMProvider themeType={themeType}>
			<CssBaseline />
...
Customizing a theme is very simple in MMStudio widgets, you just need to provide a new theme `Object`,
and all the components will change automatically.
Here is <Link target="_blank" color href="https://github.com/taoqf/widgets/tree/master/examples/custom-themes">a complete sample project</Link> for reference.
...
```jsx
import { CssBaseline, MMProvider, Themes } from '@mmstudio/widgets';
...
const App = () => (
	<MMProvider themes={[myTheme1]} themeType="coolTheme">
		<CssBaseline />
...
If you don't use TypeScript, to learn more about preset types, see <Link color target="_blank" href="https://github.com/taoqf/widgets/blob/master/components/themes/presets/index.ts">here</Link>.
...
```jsx
import { Button, Row } from '@mmstudio/widgets'
...
```jsx
import { useTheme } from '@mmstudio/widgets';
...
- `Themes.isPresetTheme` - Check if a theme is the base of MMStudio widgets.
...
- `Themes.getPresetStaticTheme` - Get the theme loaded by MMStudio widgets default.
