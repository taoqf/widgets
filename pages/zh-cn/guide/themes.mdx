...
**MMStudio widgets** 现在支持多种主题，并能以简单易用的方式随意切换或是创建新的主题，不需要任何配置与第三方库。

作为基础的选项，MMStudio widgets 内置了两种可用的基础主题，`light` 和 `dark` (亮色和暗色)。
...
<Note>
	在使用任何组件前，请确保已添加 <Code>MMProvider</Code> 至应用的根节点。
</Note>
<Spacer y={0.5} />
<Note type="success">
...
1. 确保 `MMProvider` 与 `CssBaseline` 已经添加至根节点。
...
自定义主题样式在 MMStudio widgets 中非常简单，你只需要提供一个新的样式对象给 `MMProvider`，所有的组件都会自然变化以适应你自定义的属性。
这里有 <Link target="_blank" color href="https://github.com/taoqf/widgets/tree/master/examples/custom-themes">一个完整的示例项目</Link> 可供参考。
...
如果你没有使用 TypeScript，想要了解更多的预置类型，请看<Link color target="_blank" href="https://github.com/taoqf/widgets/blob/master/components/themes/presets/index.ts">这里的类型声明文件</Link>。
...
- `Themes.isPresetTheme` - 检查一个主题对象是否来自 MMStudio widgets。
- `Themes.isAvailableThemeType` - 检查一个主题名是否可用。(是否重复)
- `Themes.hasUserCustomTheme` - 检查一组主题中是否包含自定义的主题。
- `Themes.getPresets` - 获取一组由 MMStudio widgets 内置的默认主题。
- `Themes.getPresetStaticTheme` - 获取一个由 MMStudio widgets 内置的默认主题 (默认加载的)。
